...
impl ContributorRegistryContract {
    ...
    pub fn initialize(
        ...
        // The first signer bootstraps the contract.
    ...
    /// Requires an approved `RotateSigners` proposal — but since
    /// this contract does not have that action yet, rotation is
    /// handled by the `set_multisig_config` function below which
    /// is itself guarded by an approved `SetAdmin` proposal.
    pub fn set_multisig_config(
        ...
        // Reuse SetAdmin proposal action for config rotation.
    ...
    // ── Contributor operations (unchanged) ───────────────────
    ...
    // ── Sensitive functions — now multisig-gated ─────────────

    /// Update a contributor's reputation score.
    ///
    /// Previously single-admin gated; now requires an approved
    /// `UpdateReputation` proposal before the delta is applied.
    ...
    pub fn set_admin(
        ...
        // `DataKey::Admin` is kept only for legacy read compatibility.
    ...
    /// Returns the active multisig configuration.
...
mod test {
    ...
    // ── Fixture ───────────────────────────────────────────────
    // 3-signer setup: alice (w=2), bob (w=1), carol (w=1)
    // threshold = 3  →  alice+bob OR alice+carol reach it;
    //                    bob+carol alone do not (total=2 < 3).
    ...
    fn test_propose_counts_proposer_weight() {
        ...
        assert_eq!(proposal.weight_collected, 2); // alice weight
    ...
    fn test_single_signer_above_threshold_auto_approves() {
        // If alice had weight >= threshold all by herself, propose() should
    ...
    fn test_double_sign_rejected() {
        ...
        // bob already signed via propose
    ...
    fn test_bob_carol_together_cannot_reach_threshold() {
        ...
        let id = client.propose(&s.bob, &ProposalAction::Upgrade).unwrap(); // w=1
        client.sign(&s.carol, &id).unwrap(); // w=1, total=2

        let proposal = client.get_proposal(&id).unwrap();
        assert_eq!(proposal.status, ProposalStatus::Pending); // still below 3
    }

    // ── Execute (consume_approval) ────────────────────────────
    ...
    fn test_upgrade_executes_after_approval() {
        ...
        let id = client.propose(&s.alice, &ProposalAction::Upgrade).unwrap();
        client.sign(&s.bob, &id).unwrap(); // approved

        let wasm_hash = BytesN::from_array(&s.env, &[1u8; 32]);
        // upgrade() calls consume_approval internally
        let result = client.try_upgrade(&s.alice, &id, &wasm_hash);
        // Will err on deployer in test env, but consume_approval itself passes
        let proposal = client.get_proposal(&id).unwrap();
    ...
    fn test_execute_below_threshold_fails() {
        ...
        // bob alone (w=1) proposes — threshold not met
        let id = client.propose(&s.bob, &ProposalAction::Upgrade).unwrap();
        let wasm_hash = BytesN::from_array(&s.env, &[1u8; 32]);
    ...
    fn test_wrong_action_type_rejected() {
        ...
        // Propose Upgrade but try to execute as SetAdmin
        let id = client.propose(&s.alice, &ProposalAction::Upgrade).unwrap();
    ...
    fn test_replay_blocked_after_execution() {
        ...
        // second attempt on the same proposal_id must fail
    ...
    fn test_update_reputation_succeeds_with_approval() {
        ...
        let id = client
            ...
            .unwrap();
        client.sign(&s.bob, &id).unwrap(); // threshold reached
    ...
    fn test_full_upgrade_proposal_flow() {
        ...
        // 3. Carol tries to sign an already-approved (but not yet executed) proposal — fine
        let status = client.sign(&s.carol, &id).unwrap();
        assert_eq!(status, ProposalStatus::Approved); // stays Approved, over-weight is ok

        // 4. Alice executes — proposal becomes Executed
